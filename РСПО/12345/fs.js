...
function registerUser (username, password) {
  if (users[username]) { // Проверяем, существует ли пользователь с таким именем в объекте пользователей
    console.log(`Пользователь ${username} уже существует.`); // Логгируем сообщение о том, что пользователь уже существует
  ...
  console.log(`Пользователь ${username} успешно зарегистрирован.`);
   window.location.href = 'wert.html'
  // Логгируем сообщение о успешной регистрации
...
function loginUser(username, password) {
  if (users[username] && users[username].password === password) { // Проверяем, существует ли пользователь с таким именем и паролем в объекте пользователей
    ...
    window.location.href = 'cl.html';// Перенаправляем на главную страницу
       // Обновляем интерфейс
      return true;
     // Возвращаем true, указывая на успешный вход
  }
  console.log(`Ошибка входа: неверное имя пользователя или пароль.`); // Логгируем сообщение о неудачном входе
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const registrationForm = document.getElementById('registrationForm'); // Получаем элемент формы регистрации
  const loginForm = document.getElementById('loginForm'); // Получаем элемент формы входа
